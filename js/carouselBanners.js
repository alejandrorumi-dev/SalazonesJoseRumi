...
export function setupBannerCarousel() {
	...
	// ACTUALIZADO: Usar los selectores que SÍ existen en tu HTML
	const carousel = document.querySelector('.banner-carousel-wrapper');
	const prevBtn = document.querySelector('.banner-control-btn.banner-prev');
	const nextBtn = document.querySelector('.banner-control-btn.banner-next');
	const indicators = document.querySelectorAll('.banner-indicator');
	const banners = document.querySelectorAll('.banner-promocional');
	
	if (!carousel || !prevBtn || !nextBtn || banners.length === 0) {
	...
	function goToSlide(slideIndex) {
		...
		// ACTUALIZADO: Usar clases .active como tu CSS espera
	...
	nextBtn.addEventListener('click', (e) => {
		e.preventDefault();
		e.stopPropagation();
		console.log('⏭️ Clic en botón siguiente');
		nextSlide();
	});
